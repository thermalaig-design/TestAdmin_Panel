...
const EVENTS_BUCKET = String(process.env.VITE_EVENTS_BUCKET || 'events').trim() || 'events';
const LEGACY_BUCKETS = new Set(['events', 'event', 'events_bucket']);
...
function parseStorageUrl(rawUrl, expectedBaseUrl) {
  ...
  const normalizedBucket = bucket !== EVENTS_BUCKET && LEGACY_BUCKETS.has(bucket)
    ? EVENTS_BUCKET
    : bucket;
...
async function updateEventSize(rowId, sizeKb) {
...
async function run() {
  ...
  console.log(`Total event rows: ${rows.length}`);
  ...
  for (const row of targets) {
    try {
      ...
      if (!DRY_RUN) {
        await updateEventSize(row.id, sizeKb);
      }
  ...
  console.log('--- Events compression summary ---');
