...
const DONATIONS_BUCKET = String(process.env.VITE_DONATIONS_BUCKET || 'donations').trim() || 'donations';
const LEGACY_BUCKETS = new Set(['donations', 'donation', 'donation_bucket']);
...
function parseStorageUrl(rawUrl, expectedBaseUrl) {
  ...
  const normalizedBucket = bucket !== DONATIONS_BUCKET && LEGACY_BUCKETS.has(bucket)
    ? DONATIONS_BUCKET
    : bucket;
...
async function updateDonationSize(rowId, sizeKb) {
...
async function run() {
  ...
  console.log(`Total donation rows: ${rows.length}`);
  ...
  for (const row of targets) {
    try {
      ...
      if (!DRY_RUN) {
        await updateDonationSize(row.id, sizeKb);
      }
  ...
  console.log('--- Donations compression summary ---');
