...
const NOTICEBOARD_BUCKET = String(process.env.VITE_NOTICEBOARD_BUCKET || 'noticeboard').trim() || 'noticeboard';
const LEGACY_BUCKETS = new Set(['noticeboard', 'noticeborad', 'notice_board']);
...
function parseStorageUrl(rawUrl, expectedBaseUrl) {
  ...
  const normalizedBucket = bucket !== NOTICEBOARD_BUCKET && LEGACY_BUCKETS.has(bucket)
    ? NOTICEBOARD_BUCKET
    : bucket;
...
function shouldProcessRow(row) {
  ...
  return !Number.isFinite(size) || size < 20 || size > 25;
}
...
async function updateNoticeSize(rowId, sizeKb) {
...
async function run() {
  ...
  console.log(`Total notice rows: ${rows.length}`);
  ...
  for (const row of targets) {
    try {
      ...
      for (const item of imageAttachments) {
        ...
        if (FORCE || currentBytes < TARGET_MIN_BYTES || currentBytes > TARGET_MAX_BYTES) {
      ...
      if (!DRY_RUN) {
        await updateNoticeSize(row.id, sizeKb);
      }
  ...
  console.log('--- Noticeboard compression summary ---');
