...
async function run() {
  ...
  for (const row of targets) {
    try {
      const parsed = parseStoragePublicUrl(row.photo_url, SUPABASE_URL);
      if (!parsed) {
        skipped += 1;
        console.log(`SKIPPED ${row.id} -> Unsupported photo_url`);
        continue;
      }
      ...
      updated += 1;
      console.log(
        `UPDATED ${row.id} -> ${sizeKb} KB (q=${compressed.quality}, w=${compressed.width}, range=${inRange ? 'OK' : 'NEAREST'})`
      );
    } catch (error) {
      failed += 1;
  ...
  console.log('--- Sponsors compression summary ---');
